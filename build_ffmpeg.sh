...
#apt-get -y install libsamplerate0-dev libgmp-dev libexpat-dev libpng-dev g++ libunistring-dev
...
#dnf -y install libtool-ltdl-devel libunistring-devel
...
if [ -d "$SRC/$1" ]; then
    ...
    RUST_BACKTRACE=full cargo install --root="$OUT_PREFIX" -j"${CPU_CORES}" cargo-c --locked cargo
    echo "rav1e BUILD $1"
    PATH="$OUT_PREFIX/bin:$PATH" RUST_BACKTRACE=full cargo build -j"${CPU_CORES}" --release
    PATH="$OUT_PREFIX/bin:$PATH" RUST_BACKTRACE=full cargo cinstall -j"${CPU_CORES}" --release \
          --prefix="$OUT_PREFIX"
